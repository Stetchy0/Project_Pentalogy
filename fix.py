...
print("Initializing Modular Relational Architecture Engine...")
if os.path.exists(output_dir):
    try: shutil.rmtree(output_dir)
    except Exception: pass
os.makedirs(output_dir, exist_ok=True)

# LOAD SECURE STATIC ASSETS FROM WINDOWS HIDDEN EXTENSION PATHS
...
splash_quotes_pool = ["SECURE DATABASE MAINBOARD", "ACCESS PROTOCOLS APPROVED"]
    ...
    for item in sorted(os.listdir(current_dir_path)):
        ...
        if os.path.isdir(full_path):
            if item.lower() == "bios":
                ...
                continue
            if item.startswith(".") or "templates" in item.lower() or "art" in item.lower() or "private" in item.lower():
                continue
            ...
            url = "index.html" if clean_name.lower() == "index" else (f"{clean_name.lower()}.html" if "characters" in current_dir_path.lower() or "bios" in current_dir_path.lower() else f"supp_{clean_name.lower()}.html")
            ...
            try:
                ...
                for line in lines:
                    if line.strip().startswith("|") and line.count("|") >= 4:
                        parts = [cell.strip() for cell in line.replace("\n", "").split("|") if cell.strip()]
                        if parts and "character" not in parts[0].lower() and "---" not in parts[0]:
                            if line.count("|") == 5 and len(parts) == 4:
                                c1, r12, r21, c2 = parts[0].lower().strip(), parts[1], parts[2], parts[3].lower().strip()
                            ...
                            elif line.count("|") >= 12 and len(parts) >= 11:
                                master_traits_map[parts[0].lower().strip()] = {
                                    "age": parts[1] if parts[1] else "Classified", "gender": parts[2] if parts[2] else "Classified", "sexuality": parts[3] if parts[3] else "Classified",
                                    "height": parts[4] if parts[4] else "Classified", "trope": parts[5] if parts[5] else "Classified", "motifs": parts[6] if parts[6] else "Classified",
                                    "first_ment": parts[7] if parts[7] else "Unlogged", "first_app": parts[8] if parts[8] else "Unlogged", "present_in": parts[9] if parts[9] else "Unlogged",
                                    "playlist": parts[10] if parts[10] else ""
                                }
            except Exception: pass
for root, dirs, files in os.walk(content_dir):
    for file in files:
        if file.endswith(".md"):
            ...
            for l in text.split("\n"):
                l_strip = l.strip()
                if "function showslides" in l_strip.lower() or "let slideindex" in l_strip.lower(): skip_script_block = True
